...
package Underground8::Service::ClamAV::SLAVE;
...
sub write_config
{
    ...
    open (CLAMD_LIMES,'>',$g->{'file_clamav_clamdconf'})
        or throw Underground8::Exception::FileOpen($g->{'file_clamav_clamdconf'});
...
sub service_restart ($)
{
    ...
    my $output = safe_system($g->{'cmd_clamav_restart'});
}
